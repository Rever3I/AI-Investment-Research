...
两条入口。一条是你给条件，它给名单，每个附一句入选理由。另一条是你先有判断——「电网改造的开支这几年下不来」——它顺着这条逻辑推到产业链上真正能表达它的公司。
...
报告包含生意怎么赚钱、管理层、竞争格局、市场空间，以及 8-12 条风险，每条风险后面跟着「什么情况下这个判断就是错的」。

这一段不替你下结论。它把材料铺开、把反面证据摆出来、追问你没想清楚的地方，结论留给你写——因为第五段要拿这份结论跟未来的事实对账，署名必须是你。

### 估值

**生成 DCF 估值模型，包含交互式 HTML 文件，支持实时拖动增长率、折现率。**

股东盈余 DCF 加反向 DCF。反向 DCF 报出的是当前价格已经隐含的年复合增速——可口可乐是 19.7%，这把「贵不贵」换成了一个你自己能回答的问题：一家软饮公司未来十年能不能做到这个数。

每次估值生成一个自包含的 HTML 文件。拖动折现率或永续增长率，所有情景当场重算。没有 CDN、没有外部字体、没有任何网络请求，几年后在一台断网的机器上打开照样能跑。页面还给出每个情景的永续价值占比：一个 85% 价值来自永续段的估值，说的更多是折现率而不是这门生意。

### 仓位

**使用半 Kelly 计算公式，帮助用户决策仓位数量，控制回撤。**

你给情景概率，它算权重。用的是多结果分布的 Kelly 解，不是教科书里那个二元公式，所以三档、五档情景都能直接算。

半 Kelly 是默认值，也可以切成全 Kelly、固定比例，或者你自己给权重。另有一层集中度上限在算完之后套用——Kelly 不知道你组合里还有什么，这个判断留在外面。期望收益在当前价格下不为正时，它给 0。

### 复查

**卖出前调出建仓观点，减少价格波动引起的情绪化交易，增强决策质量。**

它把当初那份 thesis 调出来，跟现在的事实对账，把变化分成三类：事实变了、判断变了、依然成立。
